...
|---|---|---|---|---|
| MOB | 触发/解除 + 页面跳转 | `mob` / `mob_cancel` | 实时广播 + 状态落库 + 导航 | ✅（`integration_test/mob_activation_navigation_sync_test.dart`） |
| MOB 规则 | 增删改开关 | `mob_rule_sync` | LWW（`updatedAt`） | ✅（`integration_test/realtime_rule_sync_test.dart`） |
| 告警设置（Safety） | 水深/速度开关与阈值 | `settings_sync` 子字段 | LWW（字段级） | ⬜ |
| 告警规则 | upsert/delete | `alarm_rules_sync` | LWW（item 级） | ✅（`integration_test/realtime_rule_sync_test.dart`） |
